...
Chat server for Windows on Snapdragon® demonstrating a large language model (LLM, e.g., [Llama 3.2 3B](https://aihub.qualcomm.com/compute/models/llama_v3_2_3b_chat_quantized)) using Genie SDK.
...
to generate `genie_bundle` required by ChatServer. If you use any of the Llama 3
...
3. Copy bundle assets from step 2 to `ChatServer\genie_bundle`. You should see
`ChatServer\genie_bundle\*.bin` context binary files.
...
Please follow the following steps to configure QAIRT SDKs for ChatServer:
...
3. If command from step 2 succeeds, QAIRT SDK is correctly configured to work with ChatServer.
...
```powershell
.\ARM64\Debug\ChatServer.exe --genie-config .\\genie_bundle\\genie_config.json --base-dir .\\genie_bundle\\
```
...
```powershell
.\ARM64\Debug\ChatServer.exe --help
```
